...
from project.database.mariadb.models.users import DeliveryData, PersonalData
...
class UserRepository(IUserRepository):
    ...
    async def get_user(self, user_id: int = None, username: str = None, email: str = None) -> Optional[Users]:
        ...
        try:
            query = select(Users).options(selectinload(Users.personal_data), selectinload(Users.delivery_data), selectinload(Users.sessions))
    ...
    async def create_user(self, user_data: dict) -> Union[str, HTTPException]:
        ...
        session_db = await self._session_db_manager.get_session()
        session_repository = SessionRepository(self._session_db_manager)
    ...
    async def login(self, user_data: dict) -> Union[str, HTTPException]:
        ...
        session_db = await self._session_db_manager.get_session()
        session_repository = SessionRepository(SessionDBManager(session_db=session_db))
